...
function Profile({ role }) {
  ...
  if (isLoading)
    return (
      <p className="text-center py-10 text-green-700">Loading profile...</p>
    );
  if (isError)
    return (
      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-6">
        ...
        <div className="bg-white border border-green-100 rounded-xl shadow-sm p-6 flex items-center gap-6">
          <div className="h-24 w-24 rounded-full bg-green-100 border-2 border-green-200 flex items-center justify-center shrink-0">
            {profile?.email ? (
              <img
                src={getGravatarUrl(profile.email, 96)}
                alt="User"
                className="h-24 w-24 rounded-full object-cover border-2 border-green-200 shrink-0"
              />
            ) : (
              <div className="h-24 w-24 rounded-full bg-green-100 border-2 border-green-200 flex items-center justify-center shrink-0">
                <svg
            ...
            {borrowedBooks.length > 0 ? (
              <ul className="flex flex-col gap-2">
                {borrowedBooks.map((book, index) => (
                  <li
                    key={index}
                    className="flex items-center gap-2 text-sm text-green-800 border-b border-green-50 pb-2 last:border-0"
                  >
                    <span className="w-2 h-2 rounded-full bg-green-500 shrink-0" />
                    <span>{book.title}</span>
                    <span className="text-gray-400">by {book.author}</span>
                  <button className="bg-green-500 hover:bg-green-600 text-white py-1 px-3 rounded-lg text-sm transition">
                    Return Book
                  </button>
                  </li>
                ))}
